...
Los dos archivos de este apartado incluyen PostgreSQL, Catalog y BFF. Para sumar Orders y su base, agregar `-f compose.orders.yml`, según su guía. Los bloques posteriores agregan Notify, RabbitMQ, Kafka, Audit y Report; ver [Report completo](REPORT_COMPLETO.md) para combinar los siete archivos. Los puertos se publican únicamente en `127.0.0.1`. No es un despliegue AWS ni una configuración lista para producción.
