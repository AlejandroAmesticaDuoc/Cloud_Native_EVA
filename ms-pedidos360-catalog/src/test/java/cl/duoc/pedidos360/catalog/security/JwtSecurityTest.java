...
import cl.duoc.pedidos360.catalog.security.support.TestJwtIssuer;
...
class JwtSecurityTest {
    ...
    void shouldAcceptSignedTokenWithTheRequiredScope() throws Exception {
        mvc.perform(get("/api/v1/catalog").header("Authorization", "Bearer " + ISSUER.sign(ISSUER.claims().build())))
    ...
    void shouldRejectInvalidClaims(String scenario) throws Exception {
        ...
        mvc.perform(get("/api/v1/catalog").header("Authorization", "Bearer " + ISSUER.sign(claims.build())))
    ...
    void shouldMapEntraRolesAndRequireScopeForAdmin() throws Exception {
        ...
        mvc.perform(delete("/api/v1/catalog/999999").header("Authorization", "Bearer " + withoutScope))
                .andExpect(status().isForbidden());
        String admin = ISSUER.sign(ISSUER.claims().claim("roles", List.of("ADMIN")).build());
        mvc.perform(delete("/api/v1/catalog/999999").header("Authorization", "Bearer " + admin))
