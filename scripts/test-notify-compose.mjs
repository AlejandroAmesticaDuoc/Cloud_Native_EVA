...
const environment = { ...process.env,
  ...
  ORDERS_DB_PASSWORD: randomBytes(24).toString('hex'), AUDIT_DB_PASSWORD: randomBytes(24).toString('hex'), REPORT_DB_PASSWORD: randomBytes(24).toString('hex'), RABBITMQ_USERNAME: 'compose-test',
...
try {
  ...
  if (withReport) {
    assert.equal(configuration.services.report.environment.KAFKA_BOOTSTRAP_SERVERS, 'kafka:19092');
    assert.equal(configuration.services.report.environment.REPORT_EVENTS_ENABLED, 'true');
    assert.equal(configuration.services.bff.environment.REPORT_SERVICE_URL, 'http://report:8084');
    assert.equal(configuration.services.report.environment.REPORT_DB_USERNAME, 'pedidos360_report');
    assert.notEqual(configuration.services.report.environment.REPORT_KAFKA_GROUP,
      configuration.services.audit.environment.AUDIT_KAFKA_GROUP);
  }
  ...
  try {
    console.error(await run([...compose, 'logs', '--no-color', '--tail', '35', 'rabbitmq', ...(withKafka ? ['kafka'] : [])]));
  } catch {}
  process.exitCode = 1;
} finally {
